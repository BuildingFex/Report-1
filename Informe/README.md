...
| EP-08 | Gestión de Gastos y Personal | Registro contable de gastos operativos y administración del directorio del personal de trabajo del edificio. |
| EP-09 | Incidencias y Mantenimiento | Reporte de averías por parte de los residentes y seguimiento del estado de resolución de los tickets. |
| EP-10 | Comunicación y Soporte | Publicación de anuncios comunitarios importantes y atención de consultas mediante chat directo. |
### USER STORIES
...
| **US-14** | Creación de perfil de condominio | **Como** administrador, **quiero** registrar un nuevo edificio, **para** empezar a gestionar sus finanzas. | **Scenario:** Registro de edificio completo.<br>**Given** que el administrador ingresa datos válidos,<br>**When** presiona guardar,<br>**Then** el sistema crea el condominio.<br><br>**Scenario:** Registro duplicado.<br>**Given** que se intenta registrar una dirección existente,<br>**When** intenta guardar,<br>**Then** el sistema alerta sobre la duplicidad. | EP-02 |
| **US-15** | Importación masiva de residentes | **Como** administrador, **quiero** subir un Excel con los datos de los vecinos, **para** agilizar la migración al sistema. | **Scenario:** Importación de lista limpia.<br>**Given** un archivo Excel correcto,<br>**When** se carga al sistema,<br>**Then** se crean las cuentas masivamente.<br><br>**Scenario:** Archivo con formato incorrecto.<br>**Given** un archivo con datos vacíos obligatorios,<br>**When** se procesa,<br>**Then** el sistema rechaza la carga y muestra los errores. | EP-02 |
| **US-16** | Facturación Automática | **Como** administrador, **quiero** que el sistema genere automáticamente la cuota cada 30 días, **para** ahorrar tiempo mensual. | **Scenario:** Generación por fecha de admisión.<br>**Given** que han pasado 30 días desde la admisión de un residente,<br>**When** el sistema ejecuta su revisión automática,<br>**Then** genera una nueva cuota en estado pendiente.<br><br>**Scenario:** Falta de fecha de admisión.<br>**Given** que un residente no tiene configurada su fecha de ingreso,<br>**When** el sistema intenta facturar,<br>**Then** omite al usuario y lanza una alerta en el panel del administrador. | EP-03 |
| **US-17** | Pago de mantenimiento en línea | **Como** residente, **quiero** pagar mi cuota digitalmente, **para** evitar hacer transferencias manuales al banco. | **Scenario:** Pago exitoso con Mercado Pago.<br>**Given** que el residente selecciona una cuota pendiente,<br>**When** completa el proceso mediante la pasarela de Mercado Pago,<br>**Then** el sistema marca el recibo como 'Pagado'.<br><br>**Scenario:** Tarjeta rechazada por Mercado Pago.<br>**Given** que la tarjeta no tiene fondos,<br>**When** se procesa el cobro,<br>**Then** la pasarela devuelve un error y la cuota sigue pendiente. | EP-04 |
| **US-18** | Conciliación automática del pago | **Como** administrador, **quiero** que el sistema concilie los abonos hechos por la web, **para** evitar cruzar comprobantes a mano. | **Scenario:** Conciliación inmediata post-pago.<br>**Given** una notificación exitosa de Mercado Pago,<br>**When** el monto coincide con la cuota,<br>**Then** se liquida la deuda automáticamente.<br><br>**Scenario:** Pago parcial o dispar.<br>**Given** que se reporta un monto diferente al facturado,<br>**When** el sistema recibe la alerta,<br>**Then** deja el recibo como 'Observado' para revisión manual. | EP-04 |
| **US-19** | Aplicación automática de mora | **Como** administrador, **quiero** que el sistema aplique una multa a los recibos vencidos, **para** penalizar los pagos tardíos. | **Scenario:** Recargo por vencimiento.<br>**Given** una cuota que superó su fecha límite,<br>**When** se ejecuta la tarea diaria,<br>**Then** el monto de mora se añade al total.<br><br>**Scenario:** Tasa de mora no configurada.<br>**Given** que no hay porcentaje de penalidad asignado,<br>**When** la cuota vence,<br>**Then** no se aplican recargos y se alerta al administrador. | EP-05 |
| **US-20** | Reserva de Espacios y Aforo | **Como** residente, **quiero** reservar áreas comunes, **para** asegurar mi turno. | **Scenario:** Reserva dentro del límite de aforo.<br>**Given** que el área común tiene cupos disponibles,<br>**When** el residente confirma la reserva,<br>**Then** el horario se bloquea a su nombre.<br><br>**Scenario:** Exceso de aforo permitido.<br>**Given** que el gimnasio ya alcanzó su capacidad máxima a las 18:00 hrs,<br>**When** un residente intenta separar ese bloque,<br>**Then** el sistema bloquea la acción indicando aforo lleno. | EP-06 |
| **US-21** | Generación de código QR | **Como** residente, **quiero** generar un código QR de invitación, **para** que mis visitas ingresen rápido. | **Scenario:** QR generado con éxito.<br>**Given** datos del invitado válidos,<br>**When** se genera el código,<br>**Then** se descarga una imagen para compartir.<br><br>**Scenario:** Código expirado.<br>**Given** un QR generado hace más de 24 horas,<br>**When** el conserje lo escanea,<br>**Then** el lector indica "Acceso Denegado". | EP-06 |
| **US-22** | Panel de Métricas (KPIs) | **Como** administrador, **quiero** ver indicadores como deuda total y ocupación, **para** medir la gestión del edificio. | **Scenario:** Visualización de estadísticas globales.<br>**Given** que el administrador abre el dashboard,<br>**When** cargan los widgets principales,<br>**Then** visualiza la deuda total, unidades ocupadas vs vacías y total de residentes.<br><br>**Scenario:** Dashboard sin historial.<br>**Given** un edificio recién creado,<br>**When** accede a la vista,<br>**Then** muestra indicadores en cero con un mensaje de bienvenida. | EP-07 |
| **US-23** | Generación de cuotas extraordinarias | **Como** administrador, **quiero** emitir cobros por imprevistos, **para** cubrir reparaciones o mejoras del edificio. | **Scenario:** Cuota extra a toda la comunidad.<br>**Given** que se aprueba un gasto imprevisto,<br>**When** se crea una cuota extraordinaria global,<br>**Then** se genera un recibo paralelo a la mensualidad.<br><br>**Scenario:** Cuota sin destinatarios.<br>**Given** que se omiten los residentes afectados,<br>**When** intenta guardar,<br>**Then** el sistema exige seleccionar al menos un departamento. | EP-03 |
| **US-24** | Exoneración de moras | **Como** administrador, **quiero** perdonar una multa por retraso, **para** manejar acuerdos con los vecinos. | **Scenario:** Condonación definitiva de recargo.<br>**Given** un recibo con penalidad,<br>**When** el administrador hace clic en "Condonar Mora",<br>**Then** se resta el recargo y el sistema bloquea recálculos futuros.<br><br>**Scenario:** Recibo ya liquidado.<br>**Given** un recibo histórico pagado totalmente,<br>**When** intenta condonar,<br>**Then** la opción permanece oculta. | EP-05 |
| **US-25** | Gestión de Gastos y Proveedores | **Como** administrador, **quiero** registrar los gastos operativos y listas de proveedores, **para** mantener la transparencia. | **Scenario:** Registro de factura a proveedor.<br>**Given** la compra de insumos de limpieza,<br>**When** se registra el gasto asociado a un proveedor recurrente,<br>**Then** se descuenta del balance de caja del mes.<br><br>**Scenario:** Monto inválido.<br>**Given** el formulario de nuevo gasto,<br>**When** ingresa un valor negativo o cero,<br>**Then** el sistema bloquea el registro pidiendo un monto válido. | EP-08 |
| **US-26** | Directorio del Equipo | **Como** administrador, **quiero** registrar al personal operativo (limpieza, conserjes), **para** gestionar sus turnos y remuneración. | **Scenario:** Ficha de empleado completa.<br>**Given** la contratación de un nuevo trabajador,<br>**When** se ingresan su DNI, foto, salario y datos de contacto,<br>**Then** se guarda en el directorio oficial del edificio.<br><br>**Scenario:** DNI duplicado.<br>**Given** que se ingresa un documento ya registrado,<br>**When** se intenta guardar,<br>**Then** el sistema detiene el proceso advirtiendo la duplicidad. | EP-08 |
| **US-27** | Inicio de Sesión Multi-rol | **Como** usuario registrado, **quiero** iniciar sesión con mis credenciales, **para** acceder a mi panel correspondiente. | **Scenario:** Redirección según rol.<br>**Given** que un usuario ingresa correo y contraseña válidos,<br>**When** presiona el botón de acceso,<br>**Then** es dirigido a la vista de Administrador o Residente dependiendo de sus permisos.<br><br>**Scenario:** Credenciales erróneas.<br>**Given** que el usuario ingresa una contraseña incorrecta,<br>**When** intenta acceder,<br>**Then** el sistema bloquea el acceso mostrando un mensaje de error genérico. | EP-02 |
| **US-28** | Perfil de Usuario | **Como** usuario, **quiero** visualizar mis datos personales o empresariales, **para** mantener mi información actualizada. | **Scenario:** Verificación de datos principales.<br>**Given** que el usuario accede a "Mi Perfil",<br>**When** carga la interfaz,<br>**Then** visualiza su DNI/RUC, dirección y correos de contacto.<br><br>**Scenario:** Actualización con campos vacíos.<br>**Given** la pantalla de edición de perfil,<br>**When** intenta borrar su número de documento principal,<br>**Then** el sistema desactiva el botón de guardar indicando que es obligatorio. | EP-02 |
| **US-29** | Monitoreo de Recaudación | **Como** administrador, **quiero** ver una lista global de las cuotas emitidas, **para** filtrar rápidamente a los morosos. | **Scenario:** Filtro por estado pendiente.<br>**Given** el módulo de recaudación,<br>**When** el administrador aplica el filtro "Pendientes",<br>**Then** la tabla muestra únicamente los recibos no pagados del mes en curso.<br><br>**Scenario:** Búsqueda sin coincidencias.<br>**Given** que se busca a un residente específico al día en sus pagos,<br>**When** se aplican los filtros de deuda,<br>**Then** la tabla muestra un estado vacío (empty state) indicando que no hay resultados. | EP-07 |
| **US-30** | Estado de Cuenta Personal | **Como** residente, **quiero** ver el historial de mis cuotas pasadas y presentes, **para** tener control de mis finanzas. | **Scenario:** Revisión del mes actual e histórico.<br>**Given** que el residente abre la sección financiera,<br>**When** revisa el listado,<br>**Then** diferencia claramente cuáles meses ya pagó y su saldo pendiente actual.<br><br>**Scenario:** Falla de conexión con la base de datos.<br>**Given** que el residente ingresa a su estado de cuenta,<br>**When** la API no responde en el tiempo esperado,<br>**Then** se muestra un mensaje temporal solicitando recargar la vista. | EP-04 |
| **US-31** | Consulta de Recibos | **Como** residente, **quiero** visualizar y descargar el comprobante de mis pagos exitosos, **para** guardarlos como respaldo. | **Scenario:** Descarga de comprobante PDF.<br>**Given** una cuota en estado 'Pagado',<br>**When** el residente presiona el ícono de descarga,<br>**Then** el sistema genera y descarga un documento PDF con los detalles del abono.<br><br>**Scenario:** Recibo no generado.<br>**Given** una cuota aún pendiente,<br>**When** el residente intenta descargar el comprobante,<br>**Then** el botón aparece deshabilitado. | EP-04 |
| **US-32** | Catálogo de Amenidades | **Como** residente, **quiero** ver las áreas comunes disponibles, **para** conocer las instalaciones que puedo utilizar. | **Scenario:** Exploración de espacios con fotos.<br>**Given** que el residente accede a la sección de servicios,<br>**When** navega por el catálogo,<br>**Then** visualiza tarjetas con fotografías y la descripción del gimnasio, piscina, etc.<br><br>**Scenario:** Sin amenidades configuradas.<br>**Given** un edificio que no ha registrado áreas comunes,<br>**When** el residente entra al módulo,<br>**Then** visualiza un mensaje indicando que no hay espacios compartidos disponibles. | EP-06 |
| **US-33** | Reporte de Incidencias | **Como** residente, **quiero** crear un ticket cuando algo falla en el edificio, **para** que el conserje o administrador lo repare. | **Scenario:** Creación de ticket detallado.<br>**Given** que el ascensor se averió,<br>**When** el residente llena el formulario con el asunto y la descripción del problema,<br>**Then** el sistema crea una incidencia nueva y notifica al administrador.<br><br>**Scenario:** Formulario sin detalle.<br>**Given** que el residente intenta enviar un reporte,<br>**When** deja el campo de descripción vacío,<br>**Then** el formulario marca el campo en rojo impidiendo el envío. | EP-09 |
| **US-34** | Seguimiento de Tickets | **Como** administrador, **quiero** actualizar el estado de una incidencia, **para** mantener informado al vecino que la reportó. | **Scenario:** Cambio de estado a 'Resuelto'.<br>**Given** un ticket abierto por fuga de agua,<br>**When** el administrador lo marca como resuelto añadiendo un comentario final,<br>**Then** el estado cambia y el residente recibe una notificación.<br><br>**Scenario:** Cierre sin comentarios.<br>**Given** un ticket abierto,<br>**When** el administrador intenta cerrarlo sin explicar la solución dada,<br>**Then** el sistema le exige escribir una breve nota de resolución. | EP-09 |
| **US-35** | Anuncios de la Comunidad | **Como** administrador, **quiero** publicar avisos en el muro principal, **para** comunicar cortes de agua o asambleas. | **Scenario:** Publicación de aviso urgente.<br>**Given** que se programó un mantenimiento eléctrico,<br>**When** el administrador crea el anuncio,<br>**Then** el mensaje aparece resaltado en el inicio de todos los residentes.<br><br>**Scenario:** Publicación vacía.<br>**Given** la herramienta de anuncios,<br>**When** intenta publicar solo con un título sin contenido,<br>**Then** el sistema bloquea la acción hasta que se agregue un cuerpo al mensaje. | EP-10 |
| **US-36** | Chat de Soporte | **Como** residente, **quiero** tener un chat directo con la administración, **para** resolver consultas sobre mis pagos rápidamente. | **Scenario:** Envío de consulta directa.<br>**Given** que el residente tiene dudas sobre su recibo,<br>**When** escribe y envía su mensaje por el chat integrado,<br>**Then** la administración recibe la alerta en tiempo real en su bandeja de entrada.<br><br>**Scenario:** Administrador fuera de línea.<br>**Given** que el residente escribe fuera de horario laboral,<br>**When** envía el mensaje,<br>**Then** el sistema emite una respuesta automática informando que será atendido a primera hora. | EP-10 |
