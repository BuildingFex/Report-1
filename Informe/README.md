...
| EP-08 | Gestión de Gastos y Personal | Registro de trabajadores y los egresos menores. |
...
| **US-14** | Creación de perfil de condominio | **Como** administrador, **quiero** registrar un nuevo edificio en el sistema, **para** empezar a gestionar sus finanzas. | **Scenario:** Registro de edificio completo.<br>**Given** que el administrador ingresa datos válidos,<br>**When** presiona guardar,<br>**Then** el sistema crea el condominio y habilita su panel.<br><br>**Scenario:** Registro duplicado.<br>**Given** que un administrador intenta registrar un edificio con la misma dirección,<br>**When** intenta guardar,<br>**Then** el sistema muestra una alerta de duplicidad. | EP-02 |
| **US-15** | Importación masiva de residentes | **Como** administrador, **quiero** subir un Excel con los datos de los vecinos, **para** no registrar departamento por departamento manualmente. | **Scenario:** Importación de lista limpia.<br>**Given** un archivo Excel con el formato correcto,<br>**When** se carga al sistema,<br>**Then** se crean las cuentas y se envían los accesos.<br><br>**Scenario:** Archivo con formato incorrecto.<br>**Given** un archivo con datos vacíos obligatorios,<br>**When** se intenta procesar,<br>**Then** el sistema rechaza la carga y muestra los errores encontrados. | EP-02 |
| **US-16** | Generación masiva de recibos | **Como** administrador, **quiero** generar los recibos de mantenimiento masivamente, **para** ahorrar tiempo al inicio de mes. | **Scenario:** Emisión mensual exitosa.<br>**Given** que se configuraron los gastos del mes,<br>**When** se genera el proceso masivo,<br>**Then** se crean recibos 'Pendientes' para todos los residentes.<br><br>**Scenario:** Generación sin configuración previa.<br>**Given** que el administrador intenta generar recibos sin asignar el presupuesto,<br>**When** hace clic en generar,<br>**Then** el sistema muestra un error indicando que falta información financiera. | EP-03 |
| **US-17** | Pago de mantenimiento en línea | **Como** residente, **quiero** pagar mi cuota usando mi tarjeta desde la plataforma, **para** evitar transferencias manuales. | **Scenario:** Pago con tarjeta aprobado.<br>**Given** que el residente ingresa datos de tarjeta válidos,<br>**When** confirma el pago,<br>**Then** el recibo cambia a 'Pagado' inmediatamente.<br><br>**Scenario:** Tarjeta rechazada.<br>**Given** que la tarjeta no tiene fondos o está bloqueada,<br>**When** se intenta procesar el cobro,<br>**Then** la pasarela devuelve un error y la deuda se mantiene pendiente. | EP-04 |
| **US-18** | Conciliación automática del pago | **Como** administrador, **quiero** que el sistema marque automáticamente como "Pagado" un recibo abonado por web, **para** evitar cruzar vouchers. | **Scenario:** Conciliación inmediata post-pago.<br>**Given** una notificación exitosa de la pasarela externa,<br>**When** el monto y el recibo coinciden,<br>**Then** el sistema liquida la deuda automáticamente.<br><br>**Scenario:** Discrepancia en el monto reportado.<br>**Given** que la pasarela reporta un cobro parcial por error externo,<br>**When** el sistema recibe la alerta,<br>**Then** deja el recibo en estado 'Observado' para revisión manual. | EP-04 |
| **US-19** | Aplicación automática de mora | **Como** administrador, **quiero** que el sistema aplique una multa a los recibos vencidos, **para** penalizar la morosidad. | **Scenario:** Recargo por vencimiento.<br>**Given** un recibo que superó su fecha límite,<br>**When** se ejecuta la revisión diaria del sistema,<br>**Then** el monto de mora se añade al total del recibo.<br><br>**Scenario:** Tasa de mora no configurada.<br>**Given** que el edificio no configuró su porcentaje de penalidad,<br>**When** llega la fecha de vencimiento,<br>**Then** el sistema no realiza recargos y notifica al administrador. | EP-05 |
| **US-20** | Reserva de áreas comunes con cobro | **Como** residente, **quiero** reservar y pagar el uso de la parrilla o SUM desde la plataforma, **para** asegurar mi turno. | **Scenario:** Reserva confirmada tras pago exitoso.<br>**Given** que el residente selecciona un horario libre para el área común,<br>**When** completa el pago de la tarifa de uso mediante la pasarela,<br>**Then** el horario queda bloqueado y se emite la confirmación de reserva.<br><br>**Scenario:** Cancelación de reserva por pago fallido.<br>**Given** que el residente selecciona el horario,<br>**When** abandona la pasarela de pago o la tarjeta es rechazada,<br>**Then** el sistema libera inmediatamente el horario para otros vecinos. | EP-06 |
| **US-21** | Generación de código QR para visitas | **Como** residente, **quiero** generar un código QR de invitación, **para** que mis visitas ingresen rápido. | **Scenario:** QR generado con éxito.<br>**Given** datos del invitado válidos,<br>**When** se genera el código,<br>**Then** se descarga una imagen lista para compartir.<br><br>**Scenario:** Código QR expirado.<br>**Given** un código generado hace más de 24 horas,<br>**When** el conserje intenta validarlo en recepción,<br>**Then** el sistema indica "Acceso Denegado: Expirado". | EP-06 |
| **US-22** | Visualización del Dashboard financiero | **Como** administrador, **quiero** ver un resumen de ingresos y morosidad, **para** conocer la salud financiera del edificio. | **Scenario:** Carga de gráficos en tiempo real.<br>**Given** que existen pagos y deudas procesadas,<br>**When** se abre el dashboard,<br>**Then** se muestran gráficos actualizados.<br><br>**Scenario:** Dashboard sin historial.<br>**Given** un edificio recién creado sin transacciones,<br>**When** el administrador accede a la vista,<br>**Then** el sistema muestra indicadores en cero con un mensaje de bienvenida. | EP-07 |
| **US-23** | Generación de cuotas extraordinarias | **Como** administrador, **quiero** emitir cobros extraordinarios, **para** cubrir gastos imprevistos como reparaciones mayores o pintura del edificio. | **Scenario:** Asignación de cuota extra a todo el edificio.<br>**Given** que se aprobó un gasto imprevisto en asamblea,<br>**When** el administrador crea una "Cuota Extraordinaria" y selecciona todos los departamentos,<br>**Then** el sistema genera un recibo adicional independiente a la cuota regular.<br><br>**Scenario:** Intento de emisión sin seleccionar departamentos.<br>**Given** que el administrador está creando el cobro extraordinario,<br>**When** olvida seleccionar los departamentos afectados y presiona generar,<br>**Then** el sistema bloquea la acción solicitando asignar destinatarios. | EP-03 |
| **US-24** | Condonación de moras | **Como** administrador, **quiero** condonar o perdonar la mora de un residente específico, **para** gestionar casos excepcionales o acuerdos de pago. | **Scenario:** Anulación exitosa de recargo.<br>**Given** que un residente justificó su retraso,<br>**When** el administrador selecciona la opción "Condonar Mora" en el recibo vencido,<br>**Then** el sistema resta el recargo y el recibo vuelve a su monto original.<br><br>**Scenario:** Intento de condonar un recibo ya pagado.<br>**Given** que un recibo histórico ya fue liquidado con todo y mora,<br>**When** el administrador intenta aplicar la condonación,<br>**Then** el sistema oculta la opción e indica que el documento está cerrado contablemente. | EP-05 |
| **US-25** | Registro de gastos operativos | **Como** administrador, **quiero** registrar los gastos diarios (artículos de limpieza, focos), **para** mantener la transparencia del flujo de caja del edificio. | **Scenario:** Registro correcto con comprobante.<br>**Given** que se realizó la compra de suministros,<br>**When** el administrador ingresa el monto, la categoría y adjunta la foto de la boleta,<br>**Then** el sistema descuenta el monto de los fondos disponibles y lo refleja en el reporte.<br><br>**Scenario:** Intento de registro con monto negativo o nulo.<br>**Given** que el administrador está llenando el formulario de gasto,<br>**When** digita un valor de "0" o inferior,<br>**Then** el sistema rechaza la entrada pidiendo un monto válido. | EP-08 |
| **US-26** | Registro de personal del edificio | **Como** administrador, **quiero** registrar al personal operativo (guardianía, limpieza), **para** controlar quiénes trabajan en las instalaciones y gestionar sus turnos. | **Scenario:** Creación de ficha de trabajador.<br>**Given** que el edificio contrató un nuevo conserje,<br>**When** el administrador ingresa sus datos personales y rol en el sistema,<br>**Then** el empleado queda listado en el directorio del edificio habilitado para el control de asistencia.<br><br>**Scenario:** Registro con DNI duplicado.<br>**Given** que se intenta ingresar a un trabajador,<br>**When** se coloca un número de documento que ya existe en la base de datos de ese edificio,<br>**Then** el sistema detiene el proceso advirtiendo sobre el registro duplicado. | EP-08 ||
